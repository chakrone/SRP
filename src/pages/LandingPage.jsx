    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#f8fafc] overflow-x-hidden relative bg-grid">
            ...
            <main className="relative flex items-center justify-center h-screen w-full">
                ...
                {/* Mobile info strip — appears at bottom of hero on small screens */}
                <div className="absolute bottom-28 left-0 right-0 z-20 flex lg:hidden justify-center gap-4 px-4">
                    <div className="flex items-center gap-3 bg-white/80 backdrop-blur-xl rounded-2xl px-5 py-3 border border-slate-100 shadow-sm">
                        <div className="text-center">
                            <div className="text-[9px] tracking-[0.2em] text-slate-400 uppercase">Price</div>
                            <div className="text-2xl font-black text-slate-900 leading-tight">3,800 <span className="text-sm font-bold text-sky-500">DH</span></div>
                        </div>
                        ...
                        <div className="flex flex-col gap-1">
                            {['Retractable Wheels', 'Regen Braking', 'Smart PCB'].map(tag => (
                                <div key={tag} className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[9px] font-semibold tracking-wide text-slate-500 uppercase">{tag}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {/* Bottom CTA */}
                <div className="absolute bottom-6 left-0 right-0 z-20 flex flex-col items-center gap-2.5 px-4">
                    <button
                        onClick={() => navigate('/order')}
                        className="group flex items-center gap-3 px-8 py-4 bg-sky-500 hover:bg-sky-600 text-white font-bold text-sm tracking-widest rounded-full transition-all duration-300 shadow-[0_8px_30px_rgba(14,165,233,0.45)] hover:shadow-[0_12px_40px_rgba(14,165,233,0.6)] hover:scale-105 uppercase w-full max-w-xs justify-center"
                    >
                        Order Now
                        <ChevronRight size={18} className="group-hover:translate-x-1 transition-transform stroke-[2.5px]" />
                    </button>
                    <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-slate-400 text-[10px] tracking-[0.15em] uppercase">
                ...
                <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14 md:py-20">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                        {FEATURES.map(({ icon: Icon, label, desc }) => (
                            <div key={label} className="group p-4 md:p-6 rounded-2xl bg-slate-50 border border-slate-100 hover:border-sky-200 hover:bg-sky-50/50 hover:shadow-[0_0_30px_rgba(14,165,233,0.08)] transition-all duration-300">
                                <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-sky-100 flex items-center justify-center mb-3 md:mb-4 group-hover:bg-sky-200 transition-colors">
                                    <Icon size={18} className="text-sky-600" />
                                </div>
                                <div className="text-xs md:text-sm font-bold text-slate-800 mb-1">{label}</div>
                ...
                <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 md:py-10">
                    <div className="grid grid-cols-3 gap-4 md:gap-8 text-center">
                        {[
                            ...
                            <div key={label}>
                                <div className="text-2xl md:text-4xl font-black text-slate-900">{val}</div>
                                <div className="text-[10px] md:text-xs font-bold text-sky-500 tracking-wider mt-1 uppercase">{label}</div>
            ...
            <section className="relative z-20 border-t border-slate-100 bg-white">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 md:py-8">
                    <div className="grid grid-cols-2 md:flex md:flex-wrap items-center justify-center gap-4 md:gap-8 text-slate-400 text-xs">
                        {[
